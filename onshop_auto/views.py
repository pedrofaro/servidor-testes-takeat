...
from .models import CategoriaAuto, AtributoAuto, ProdutoAuto, PerguntaAuto, RespostaAuto, ComplementoModeloAuto, PerguntaModeloAuto, RespostaModeloAuto, Mesa, Codigo, Comanda, ComandaSession, PedidoAuto, ProdutoPedidoAuto, CompradorPedidoAuto, MensagemAuto, ItemPedidoAuto, QAvaliado, Avaliacao, Garcom, FormaPagamento, Conciliacao
from .forms import CategoriaForm, ProdutoForm, PerguntaForm, RespostaForm, ComplementoModeloForm, PerguntaModeloForm, RespostaModeloForm, AtribuirForm, AtributoForm, MesaForm, CodigoForm, ProdutoMobileForm, ContatoForm, MensagemAutoForm, MesaAbertaForm, MesasForm, AvaliacaoForm, NotaAvaliacaoForm, GarcomForm, AtendimentoForm, FormaPagamentoForm, ConciliaForm
...
def ver_relatorios(request):
	'''
	Página dentro do Auto para relatórios padrões (não premiuns)
	'''
...
# ------------ Views para Controle de Caixa -----------
# ------------ -------------------    
@login_required
def formas_pagamento(request):
	'''
	View para a Formas de Pagamento dentro do Painel
	'''
	pedidos = PedidoAuto.objects.filter(status=PedidoAuto.PEDIDO_NOVO)
	qtde_novos_pedidos = pedidos.count()
	...
	return render(request, 'onshop_auto/formas_pagamento.html', {'formas': formas, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})

@login_required
def editar_formas_pagamento(request, id= None):
	if id:
		auxiliar = get_object_or_404(FormaPagamento, id=id)

	if request.method == 'POST':
		if id:
			forma = forma_form = FormaPagamentoForm(request.POST, instance=auxiliar)
		else:
			forma = forma_form = FormaPagamentoForm(request.POST, request.FILES)

		if forma_form.is_valid():
			forma = forma_form.save(commit=False)
			forma.save()
			
			return redirect('onshop_auto:formas_pagamento')

	else:
		if id:
			forma_form = FormaPagamentoForm(instance=auxiliar)
		else:
			forma_form = FormaPagamentoForm()

	return render(request, 'onshop_auto/form_forma_pagamento.html', {'form': forma_form})

@login_required
def deletar_forma_pagamento(request, id):
...
@login_required
def conciliar_comanda(request, id_comanda):
	comanda = get_object_or_404(Comanda, id=id_comanda)
	mesa = comanda.mesa

	conciliacoes = Conciliacao.objects.filter(comanda= comanda)

	total_conciliado = Decimal(0)

	for aux in conciliacoes:
		total_conciliado = Decimal(total_conciliado) + Decimal(aux.valor_pago)
	...
	total = comanda.total
	servicos = total * Decimal(0.1)
	total_servicos = total * Decimal(1.1)

	return render(request, 'onshop_auto/conciliar_comanda.html', {'total': total, 'total_conciliado': total_conciliado, 'servicos': servicos, 'total_servicos': total_servicos, 'mesa': mesa, 'comanda': comanda, 'conciliacoes': conciliacoes, 'itens':itens_pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})

@login_required
def adicionar_conciliacao(request, id_comanda):
	comanda = get_object_or_404(Comanda, id=id_comanda)
	mesa = comanda.mesa
	...
	if request.method == 'POST':
		concilia = concilia_form = ConciliaForm(request.POST, request.FILES)

		if concilia_form.is_valid():
			concilia = concilia_form.save(commit=False)
			concilia.comanda = comanda
			concilia.save()
			
			return redirect('onshop_auto:conciliar_comanda', id_comanda=id_comanda)
	else:
		concilia_form = ConciliaForm()

	return render(request, 'onshop_auto/concilia_pagamento.html', {'form': concilia_form, 'total': total, 'total_conciliado': total_conciliado, 'servicos': servicos, 'total_servicos': total_servicos, 'mesa': mesa, 'comanda': comanda, 'conciliacoes': conciliacoes, 'itens':itens_pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})

@login_required
def deletar_conciliacao(request, id):
	conciliacao = get_object_or_404(Conciliacao, id=id)
	id_comanda = conciliacao.comanda.id
...
@login_required
def comanda_conciliada(request, id_comanda):

	comanda = get_object_or_404(Comanda, id=id_comanda)
	comanda.conciliada = Comanda.STATUS_SIM
	comanda.save()
